...
class Core
{
    ...
    function Route()
    {
        ...
        if(!empty($_GET['url'])){
            ...
            //获取控制器名
            $controllerName = ucfirst($urlArray[0]);//将字符串的首字母转换为大写
            //获取动作名
...
echo $controller;
        ...
        if((int)method_exists($controller, $action)){
            call_user_func_array(array($dispath, $action), $queryString);//调用回调函数，并把一个数组参数作为回调函数的参数
        }else{
    ...
    function removeMagicQuotes()
    {
        if(get_magic_quotes_gpc()){
            $_GET = stripSlashesDeep($_GET);//反引用一个引用字符串
    ...
    //自动定义全局变量（register globals） 并移除
    function unregisterGlobals()
    {
        if(ini_get('register_globals')){
            $array = array('_SESSION','_POST','_COOKIE','_REQUEST','_SERVER','ENV','FILES');
